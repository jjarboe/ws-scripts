# For 64-bit builds of matplotlib and numpy, try
...
#def bar_graph(ax, stacked=False, series=[], log=False):
#    colors = itertools.cycle(['#b04020', '#40b020', '#2040b0', '#b0b020', '#20b0b0', '#b020b0', '#ff40b0', '#b0ff40', '#40b0ff'])
#
#    # Determine and validate the number of groups
#    N = series[0].length()
#    for s in series[1:]:
#        if s.length() != N:
#            raise Exception('Data series must all have the same length!')
#
#    # Map out the x locations for the groups
#    ind = np.arange(N)
...
#
#    r = []
#    pos = np.zeros(N)  # keep track of the current total for stacked charts
...
#        height = s.data()
#        color = colors.next()
#        # Ugly computation for the width of the chart
#        #lim =  [ax.spines['left'].properties()['extents'].get_points()[0][0], ax.spines['right'].properties()['extents'].get_points()[0][0]]
#        #barw = (lim[1]-lim[0])/20
#        attrib = {
#            #'yerr': s.err(),
#            'log': log,
...
#            'edgecolor': 'white',
#            #'error_kw': {
#            #    # Make the error bars lighter than the main data so we can see them
#            #    'ecolor': mc.rgb2hex(((np.array(mc.hex2color(color))*255).astype('uint8')|(np.array(mc.hex2color('#888888'))*255).astype('uint8'))/255.),
#            #    # Taper the error bar width by index, so we can see everything if they overlap
#            #    #'elinewidth': max(3,barw-i*2),
#            #}
#        }
#        if stacked:
#            # Place stacked bars on top of each other
#            attrib.update({'bottom': pos})
#        else:
#            # Place unstacked bars next to each other
#            left = left + width * i
#            #attrib['error_kw']['elinewidth'] = barw/3
#        
#        #attrib.update({'width': width})
#        attrib.update({'height': height})
#        attrib.update({'align': 'center'})
#        #r.append(ax.bar(left, **attrib))
#        r.append(ax.bar(ind, **attrib))
#        pos = pos + s.data()
#
#    # add some labels
#    #ax.set_xticks(ind + W/2. + 0.2)
#    ax.set_xticks(ind)
#    
#    ax.legend( [x[0] for x in r], [x.name for x in series], loc=0 )
#
#    def autolabel(rects):
...
#            if height:
#                ax.text(rect.get_x()+rect.get_width()/2., rect.get_y()+height/2., '%d'%(height,),
#                    ha='center', va='bottom')
#                #rect.label = '%d' % int(height)
#
#    for rect in r:
#        autolabel(rect)
#
...
#def make_chart(filename, title, series, categories, ylabel='Scores', format='column|stacked'):
#    if '|stacked' in format:
#        stacked = True
#        format = format.replace('|stacked','')
#    else:
#        stacked = False
#    if '|log' in format:
#        yscale = 'log'
#        format = format.replace('|log','')
#    else:
#        yscale = 'linear'
#
...
#
#    if format != 'column':
#        raise Exception('Unrecognized chart format "%s"' % (format,))
#
#    fig = plt.figure()
#    plt.autoscale(True, 'y', True)
#    ax = fig.add_subplot(1, 1, 1)
#    g = bar_graph(ax, stacked=stacked, series=series.values(), log=(yscale=='log'))
...
#        f_format = 'pdf'
#    elif '.svg' in filename.lower():
#        f_format = 'svg'
#    else:
#        raise Exception('Unsupported chart output format "%s"' % (f_format,))
#
#    #g.savefig('test.svg', format='svg')
#    g.savefig(filename, format=f_format)
    
#def main():
#    series = dict_to_series(odict([
##     ('Men', ((20,2), (35,3), (30,4), (35,1), (27,2))),
#     ('Women', ((25,3), (32,5), (34,2), (20,3), (25,3))),
#     ('Other', ((5,3), (2,5), (4,2), (2,3), (5,3))),
#     ('Unk', (13,12,15, 11, 7)),
#    ]))
#    categories = ('G1', 'G2', 'G3', 'G4', 'G5')
#    
#    make_chart('test.png', 'Scores by group and gender', series, categories)
#
...
class Template(Templite):
    ...
    from WebService import SourceFile, CheckerDescription, Component
    ...
    def __call__(self, __namespace=None, **kw):
        #from cim_charts import make_chart
